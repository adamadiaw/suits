// frontend/src/components/shared/Stars.jsx

function Stars({ rating, size = 'md', showNumber = false }) {
  const sizes = {
    sm: 'w-3 h-3',
    md: 'w-4 h-4',
    lg: 'w-5 h-5',
    ...
    <div className="flex items-center gap-1">
      <div className="flex items-center gap-0.5">
        {[...Array(5)].map((_, i) => {
          if (i < fullStars) {
            return (
              <StarFilled key={i} className={sizes[size]} />
            );
          } else if (i === fullStars && hasHalfStar) {
            return (
              <StarHalf key={i} className={sizes[size]} />
            );
          } else {
            return (
              <StarEmpty key={i} className={sizes[size]} />
            );
          }
        })}
      </div>
...
function StarHalf({ className }) {
  return (
    <svg className={`${className} text-yellow-400`} viewBox="0 0 20 20">
      <defs>
